...
const STORAGE_KEY = 'epstein_nexus_v15_state';
...
export default function App() {
  ...
  const processDocumentAgent = async (docId: string) => {
    ...
    try {
      ...
      const activeChain = state.config.priority.filter(p => state.config.enabled[p]);
      if (activeChain.length === 0) throw new Error("No intelligence providers enabled.");

      for (const provider of activeChain) {
        try {
          lineage.push(provider);
          if (provider === 'gemini') {
            analysis = await analyzeDocument(text, images, state.config.geminiModel);
          } else if (provider === 'openrouter' && state.config.openRouterKey) {
            analysis = await analyzeWithOpenRouter(text, images, state.config.openRouterKey, state.config.openRouterModel);
          } else if (provider === 'lmstudio') {
            analysis = await analyzeWithLMStudio(text, state.config.lmStudioEndpoint);
          }
      ...
      if (!analysis) throw new Error("All active intelligence providers failed.");

      if (state.config.dualCheckMode && analysis.entities.some(e => e.isFamous)) {
        ...
        if (remainingEnabled.length > 0) {
          ...
          try {
            const verification = secondProvider === 'openrouter' 
              ? await analyzeWithOpenRouter(text, images, state.config.openRouterKey, state.config.openRouterModel)
              : secondProvider === 'gemini'
                ? await analyzeDocument(text, images, state.config.geminiModel)
                : await analyzeWithLMStudio(text, state.config.lmStudioEndpoint);
            
            if (verification) {
              analysis.keyInsights = [...new Set([...analysis.keyInsights, ...verification.keyInsights])];
              analysis.entities = [...analysis.entities, ...verification.entities.filter(ve => !analysis!.entities.some(e => e.name === ve.name))];
      ...
      analysis.entities.forEach((e: Entity) => {
        if (e.isFamous || e.role.toLowerCase().includes('agent') || e.role.toLowerCase().includes('witness')) {
          ...
          if (existing) {
            if (!existing.mentions.some(m => m.docId === docId)) existing.mentions.push({ docId, docName: doc.name, context: e.context });
          } else {
            updatedPois.push({
              id: Math.random().toString(36).substr(2, 9),
              name: e.name, mentions: [{ docId, docName: doc.name, context: e.context }],
      ...
      setState(prev => ({
        ...prev,
        documents: prev.documents.map(d => d.id === docId ? { ...d, content: text, images, analysis: analysis!, status: 'completed', isPOI: analysis!.entities.some(ent => ent.isFamous), lineage } : d),
        pois: updatedPois,
        processingQueue: prev.processingQueue.filter(id => id !== docId)
      }));
    } catch (err: any) {
      updateDocStatus(docId, 'error');
      showToast(`Crit-Error: ${err.message.substring(0, 50)}`, "error");
      setState(prev => ({ ...prev, processingQueue: prev.processingQueue.filter(id => id !== docId) }));
    } finally { setActiveAgentsCount(prev => prev - 1); }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setState(prev => ({ ...prev, documents: [...prev.documents, ...newDocs], processingQueue: [...prev.processingQueue, ...queueIds] }));
  };

  const resetArchive = () => {
    if (confirm("Reset Archive?")) {
      fileStore = {};
      setState(prev => ({ ...prev, documents: [], pois: [], processingQueue: [], chatHistory: [{ role: 'system', content: 'Database Zeroed.', timestamp: Date.now() }] }));
  ...
  const handleChat = async () => {
    ...
    } catch { setState(prev => ({ ...prev, chatHistory: [...prev.chatHistory, { role: 'system', content: 'Inference failure.', timestamp: Date.now() }] })); }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard title="Shards" value={state.documents.length} icon={<Database />} color="text-blue-500" />
        <StatCard title="Targets" value={state.pois.length} icon={<Users />} color="text-indigo-400" />
        <StatCard title="Analyzed" value={state.documents.filter((d: any) => d.status === 'completed').length} icon={<RefreshCw />} color="text-green-500" />
        <StatCard title="Queued" value={state.processingQueue.length} icon={<Loader2 />} color="text-amber-500" />
      </div>
            ...
            {doc.status === 'completed' ? (
              <div className="space-y-6">
                <section>
                  ...
                  <p className="text-slate-200 font-medium italic text-sm border-l-2 border-indigo-600 pl-4 py-1 leading-relaxed">{doc.analysis?.summary}</p>
                </section>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-6 space-y-8 shadow-xl backdrop-blur-xl">
        ...
        <div className="space-y-6">
          <section className="space-y-3">
            <h3 className="text-[8px] font-black text-indigo-500 uppercase tracking-widest flex justify-between">
              <span>Execution Priority & Enablement</span>
            </h3>
            <div className="space-y-2">
          ...
          <section className="p-4 bg-indigo-600/5 border border-indigo-500/20 rounded-xl flex justify-between items-center">
            <div className="flex items-center gap-3"><ShieldCheck className="w-5 h-5 text-indigo-500" /><div><div className="font-black text-[10px] uppercase text-white">Cross-Verification Mode</div><div className="text-[7px] text-slate-500 uppercase tracking-widest">Double check POIs using secondary model</div></div></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             ...
             <div className={`p-4 bg-slate-950/40 border border-slate-800 rounded-xl space-y-4 transition-all ${localConfig.enabled.gemini ? 'border-indigo-500/20' : 'grayscale opacity-30 pointer-events-none'}`}>
                ...
                <div className="space-y-1">
                  ...
                  <select className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[9px] uppercase font-bold outline-none focus:border-indigo-500" value={localConfig.geminiModel} onChange={e => setLocalConfig({...localConfig, geminiModel: e.target.value})}>
                    {GEMINI_PRESETS.map(m => <option key={m} value={m}>{m}</option>)}
                    <option value="custom">-- Custom Override --</option>
                  </select>
                  {localConfig.geminiModel === 'custom' && (
                    <input type="text" placeholder="Enter model ID..." className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[9px] mt-1 outline-none focus:border-indigo-500" onBlur={e => setLocalConfig({...localConfig, geminiModel: e.target.value})} />
                  )}
                </div>
             </div>
             ...
             <div className={`p-4 bg-slate-950/40 border border-slate-800 rounded-xl space-y-4 transition-all ${localConfig.enabled.openrouter ? 'border-indigo-500/20' : 'grayscale opacity-30 pointer-events-none'}`}>
                ...
                <div className="space-y-1">
                  <label className="text-[7px] font-black uppercase text-slate-500">Active Model</label>
                  <select className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[9px] uppercase font-bold outline-none focus:border-indigo-500" value={localConfig.openRouterModel} onChange={e => setLocalConfig({...localConfig, openRouterModel: e.target.value})}>
                    {OPENROUTER_PRESETS.map(m => <option key={m} value={m}>{m}</option>)}
                    <option value="custom">-- Custom Override --</option>
                  </select>
                  {localConfig.openRouterModel === 'custom' && (
                    <input type="text" placeholder="Enter full model string..." className="w-full bg-slate-900 border border-slate-800 rounded p-1.5 text-[9px] mt-1 outline-none focus:border-indigo-500" onBlur={e => setLocalConfig({...localConfig, openRouterModel: e.target.value})} />
                  )}
                </div>
             </div>
          </div>
...
function StatCard({ title, value, icon, color }: any) {
  return (
    <div className="bg-slate-900/40 p-4 rounded-xl border border-slate-800/60 shadow-lg">
